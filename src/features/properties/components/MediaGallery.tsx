...
const MediaGallery: React.FC<MediaGalleryProps> = ({
  ...
  useEffect(() => {
    ...
    async function loadMedia() {
      ...
      try {
        ...
        const result = await mediaService.getMediaByProperty(
          propertyId,
          options,
        );
  ...
  async function handleFileDrop(acceptedFiles: File[]) {
    ...
    try {
      const uploadedItems = await mediaService.uploadMediaFiles(
        propertyId,
        acceptedFiles,
      );
  ...
  const MediaItemComponent: React.FC<{
    ...
    useEffect(() => {
      if (item.fileType === "image") {
        mediaService
          .optimizeImage(item.url, { width: 300, height: 200 })
          ...
          {uploading ? (
            ...
            <div>
              ...
              <p className="text-gray-600 mb-1">
                Arrastre imágenes o videos aquí, o haga clic para seleccionar
              </p>
      ...
      {loading && mediaItems.length === 0 ? (
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-gray-500 text-center">
            Mostrando {mediaItems.length} de {totalCount}{" "}
