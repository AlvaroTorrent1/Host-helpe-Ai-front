  ...
  return (
    <div className="bg-white rounded-lg shadow-xl overflow-hidden">
      ...
      <div className="relative h-64 w-full">
        <img 
          src={property.image || 'https://via.placeholder.com/800x400?text=Sin+imagen'} 
          alt={property.name} 
            ...
            {property.additional_images && property.additional_images.length > 0 ? (
              <div className="space-y-6">
                <p className="text-sm text-gray-500">
                  Estas imágenes adicionales se mostrarán al turista cuando consulte información específica de la propiedad a través del chatbot.
                </p>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
                  {property.additional_images.map((image) => (
                    <div key={image.id} className="group border rounded-md p-3 bg-white hover:shadow-md transition">
                      <div 
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 flex items-center justify-center transition-opacity">
                          <svg className="h-8 w-8 text-white opacity-0 group-hover:opacity-100" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zm-3 0v3m0 0v3m0-6h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                        </div>
                      </div>
                      <div className="mt-2">
                        <h4 className="text-sm font-medium text-gray-900">Descripción</h4>
                        <p className="text-sm text-gray-500">{image.description || 'Sin descripción'}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="mt-4 text-gray-500">No hay imágenes adicionales para mostrar.</p>
                <p className="mt-2 text-sm text-gray-400">Puedes añadir imágenes adicionales con descripciones al editar la propiedad.</p>
              </div>
            )}
          </div>
        )}
      ...
      {enlargedImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75" onClick={() => setEnlargedImage(null)}>
          <div className="max-w-3xl max-h-full p-4" onClick={e => e.stopPropagation()}>
            <img src={enlargedImage} alt="Imagen ampliada" className="max-w-full max-h-[85vh] object-contain" />
            <button 
              className="absolute top-4 right-4 p-2 bg-white/80 rounded-full"
