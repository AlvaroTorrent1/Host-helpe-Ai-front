...
const PropertyDocumentsForm: React.FC<PropertyDocumentsFormProps> = ({ 
  ...
  const handleUpload = async () => {
    if (!currentDocument.file || !currentDocument.name || !currentDocument.type) {
  ...
  const isTemporaryDocument = (document: PropertyDocument): boolean => {
    return document.property_id === 'temp' && document.file_url.startsWith('data:');
  };
                ...
                <div>
                  ...
                  {isTemporaryDocument(doc) && (
                    <p className="text-xs text-amber-600 mt-1">
                      Documento temporal - Se guardará cuando finalice la creación de la propiedad
                    </p>
                  )}
                </div>
                <button
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setCurrentDocument({...currentDocument, type: e.target.value as 'faq' | 'guide' | 'house_rules' | 'inventory' | 'other'})}
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm"
            >
        ...
        {propertyId === 'temp' && (
          <div className="mt-3 text-sm text-amber-600 bg-amber-50 p-3 rounded-md">
            <p className="flex items-center">
              ...
              Los documentos se guardarán temporalmente hasta que finalices la creación de la propiedad.
            </p>
          </div>
        )}
      </div>
