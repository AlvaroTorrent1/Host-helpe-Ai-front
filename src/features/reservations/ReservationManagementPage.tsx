...
const ReservationManagementPage: React.FC = () => {
  ...
  const [currentReservation, setCurrentReservation] = useState<Reservation | undefined>(undefined);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Usamos datos de ejemplo para el MVP
        ...
        setErrorMessage('Error al cargar los datos. Por favor, inténtalo de nuevo más tarde.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleCreateReservation = (data: ReservationCreateData) => {
    ...
    setTimeout(() => {
      try {
        ...
        const additionalGuests: Guest[] = (data.additionalGuests || []).map(guest => ({
        ...
        setErrorMessage('Error al crear la reserva. Por favor, inténtalo de nuevo.');
      } finally {
  ...
  const handleUpdateReservation = (data: ReservationCreateData) => {
    ...
    setTimeout(() => {
      try {
        ...
        const existingAdditionalGuests = currentReservation.guests.filter(g => g.id !== currentReservation.mainGuestId);
        
        // Preparar huéspedes adicionales nuevos y existentes
        const additionalGuests: Guest[] = (data.additionalGuests || []).map((guest, index) => {
        ...
        setReservations(prev => 
          prev.map(r => r.id === currentReservation.id ? updatedReservation : r)
        );
        ...
        setErrorMessage('Error al actualizar la reserva. Por favor, inténtalo de nuevo.');
      } finally {
        setIsSubmitting(false);
      }
    }, 1000); // Simular latencia de red
  ...
  const handleSendToSES = () => {
    ...
    setTimeout(() => {
      try {
        ...
        setCurrentReservation(updatedReservation);
        setSuccessMessage('Datos enviados correctamente al Sistema de Entrada de Viajeros (SES)');
      } catch (error) {
        console.error('Error al enviar datos a SES:', error);
        setErrorMessage('Error al enviar datos al SES. Por favor, inténtalo de nuevo más tarde.');
      } finally {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="mb-6">
          <ol className="flex space-x-2 text-sm text-gray-500">
            ...
            <li>
              <Link to="/dashboard/reservations" className={`${viewMode !== ViewMode.LIST ? 'hover:text-primary-600' : 'text-gray-800 font-medium'}`}>
                Reservas
              </Link>
            </li>
        ...
        {(errorMessage || successMessage) && (
          <div className={`rounded-md p-4 mb-6 ${errorMessage ? 'bg-red-50' : 'bg-green-50'}`}>
            <div className="flex">
              ...
              <div className="ml-3">
                <p className={`text-sm font-medium ${errorMessage ? 'text-red-800' : 'text-green-800'}`}>
                  {errorMessage || successMessage}
                </p>
              </div>
              ...
              <div className="bg-white shadow overflow-hidden sm:rounded-lg">
                ...
                <div className="px-4 py-5 sm:p-6">
                  <ReservationForm
                    reservation={currentReservation}
                    properties={properties}
                    onSubmit={currentReservation ? handleUpdateReservation : handleCreateReservation}
