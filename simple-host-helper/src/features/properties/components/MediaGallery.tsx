...
import mediaService, { MediaItem, PaginationOptions } from '../../../services/mediaService';
...
const MediaGallery: React.FC<MediaGalleryProps> = ({ propertyId, editable = false }) => {
  ...
  function renderMediaItem(item: MediaItem) {
    return (
      <div key={item.id} className="relative rounded-lg overflow-hidden bg-gray-100 group">
        {item.fileType === 'image' ? (
          <img 
            src={mediaService.optimizeImage(item.url, { width: 300, height: 200 })} 
            alt={item.fileName}
